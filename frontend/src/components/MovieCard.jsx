...
const MovieCard = ({ movie, displayPopUpDateReservation }) => {
  const navigate = useNavigate();

  return (
    <div
      onClick={() => navigate(`./${movie.id}`)}
      className="max-w-sm rounded overflow-hidden shadow-lg bg-gray-800 text-white cursor-pointer mb-6 transition-transform transform hover:scale-105"
    >
      ...
      <div className="px-6 py-4 flex flex-wrap justify-center gap-2">
      ...
      <div className="px-6 py-4 flex justify-end">
        <button
          onClick={(e) => {
            e.stopPropagation();
            displayPopUpDateReservation(movie.id);
          }}
